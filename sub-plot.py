...
def run_plotting(name):
    ...
    if not os.path.exists(datadir):
        os.makedirs(datadir)
    chartsdir = os.path.join(root, 'charts-tmp')
    ...
    for index, algo in enumerate(algonames):
        ...
        # Cluster
        clusterdf = []
        df = pd.read_csv(datain +'acc.csv', index_col=0).T
        cols = list(df.columns)
        cols = [col + ' acc' for col in cols]
        df.columns = cols
        clusterdf.append(df.copy())

        clusterdf = pd.concat(clusterdf,1)
        clusterdf.index = clusterdf.index.astype(int)
        clusterdf = clusterdf.rename(columns = lambda x: x.replace('acc','Cluster Accuracy'))
        ...
        for clust in ['GMM','Kmeans']:
            ...
            cols = list(df.columns)
            cols = [clust + ' ' + 'NN CV Accuracy' for col in cols]
            #df.columns = [clust]
            df.columns = cols

            clusterdf = pd.concat([clusterdf, df], 1)
        plot_data(clusterdf, ['b--','g--','b-','g-'], '{} Cluster Accuracy and NN CV Accuracy.'.format(capitalname), "Clusters", outfile = out + 'ClusterScores.png', loc='center right')
...
def plot_data(df, style=None, title="", xlabel=None, ylabel=None, outfile = '', loc='upper right', label=None):
    #ax = df.plot(title=title, fontsize=20, style=style, label=label)
    ax = df.plot(style=style, label=label, fontsize=18)
    ax.set_title(title, fontsize=20)
    if xlabel is not None:
        ax.set_xlabel(xlabel, fontsize=18)
    if ylabel is not None:
        ax.set_ylabel(ylabel, fontsize=18)
    ax.legend(loc=loc, framealpha=0.5, fontsize=18)
