...
def run_plotting(name):
    ...
    for index, algo in enumerate(algonames):
        ...
        # Runtime
        #df = pd.read_csv(datain + 'runtimes.csv', index_col=0)
        #displayname = '{0} {1} '.format(capitalname, algo.upper())
        #plot_data(df.T, ['bx-'], displayname + 'Runtime vs. clusters', "Clusters", "Runtime (ms)", out + 'runtimes.png')
        ...
        # Adjusted Mutual Information
        #df = pd.read_csv(datain + 'silhouette_score.csv', index_col=0)
        #displayname = '{0} {1} '.format(capitalname, algo.upper())
        #plot_data(df.T, None, displayname + 'Silhouette Score', "Clusters", "Silhouette Score", out + 'Silhouette.png', loc='lower right')

        # Adjusted Mutual Information
        #df = pd.read_csv(datain + 'hgenity_score.csv', index_col=0)
        #displayname = '{0} {1} '.format(capitalname, algo.upper())
        #plot_data(df.T, None, displayname + 'Homogeneity Score', "Clusters", "Homogeneity Score", out + 'Homogeneity.png', loc='lower right')
        ...
        #df = pd.read_csv(datain + '2D.csv', index_col=0, dtype={'target':int, 'KmeansCluster':int, 'GMMCluster':int})
